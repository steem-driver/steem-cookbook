...
class SteemComment:
    ...
    def get_url(self, host=None):
        if self.url is None or host is not None:
            host = host if host is not None else STEEM_HOST
            if self.author_perm:
                self.url = u"{}/{}".format(host, self.author_perm)
            else:
                c = self.get_comment()
                if c.authorperm:
                    self.url = u"{}/{}".format(host, c.authorperm)
                else:
                    self.url = u"{}/@{}/{}".format(host, c.author, c.permlink)
