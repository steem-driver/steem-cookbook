...
class VoteBot:
    ...
    def sleep(self):
        wait_time = round(MINIMUM_VOTE_INTERVAL * 1.2 + random.random() * MINIMUM_VOTE_INTERVAL * 0.3, 2)
    ...
    def watch(self, ops):
        ...
        self.ctx(ops)
        if self.who_to_vote(author) and self.what_to_vote(ops) and self.is_ready():
    ...
    def run(self):
        if self.mode.startswith("stream."):
            ...
            elif self.mode == "query.vote.upvote":
                self.config['mode'] = "vote"
                self.config['pattern'] = True
            elif self.mode == "query.vote.downvote":
                self.config['mode'] = "vote"
                self.config['pattern'] = False
            elif self.mode == "query.vote.all":
                self.config['mode'] = "vote"
